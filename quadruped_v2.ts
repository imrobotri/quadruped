...
namespace Quadruped {
    ...
    */
    ...
    //###Control action||控制动作组
    /**
    *TODO:Select a different action group and enable it. Action group one is a New Year's greeting action, action group two is a stretching waist action, and action group three to ten has not been set.
    *TODO:选择不同的动作组并进行使能。动作组一为拜年动作，动作组二为伸懒腰动作，动作组三到十暂未设定。
    */
    //% group="control"
    //% blockGap=8
    //% choose.min=1.00 choose.max=10.00
    //% blockId=Quadruped_Action block="Action_choose | %choose | Action_enable | %e"
    export function Action_enable(choose: number, e: action): void {
        switch (e) {
            case action_enable = 1:   //使能
                Standing()            //站立
                action_choose = 1;    //动作组选择1
                SPI_Send()            //发送数据
                return
            
            case action_enable = 1:   //使能
                Standing()            //站立
                action_choose = 2;    //动作组选择2
                SPI_Send()            //发送数据
                return
            
            case action_enable = 0:   //不使能
                return
        }
    }
    //###gait||步态
